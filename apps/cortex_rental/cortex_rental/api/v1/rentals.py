...
def _serialize(doc) -> Dict[str, Any]:
    ...
    return {
        ...
        "discount_total": sum(
            PricingService.calculate_line_total(
                float(row.rate or 0), float(row.qty or 0), float(row.billable_days or 0)
            )
...
if frappe:
    ...
    def create_quote_draft():
        ...
        name = with_idempotency(
            company=company,
            scope="rentals.create_quote_draft",
        ...
        if version is not None and int(version) != int(doc.version or 1):
            return {
                ...
                "errors": [
                    {
                        "code": "stale_version",
                        "message": "La location a changé. Recharge-la avant de confirmer.",
                    }
                ],
            }
    ...
    def request_contract(name: str, version: int = None, override_reason: str = None):
        ...
        if version is not None and int(version) != int(doc.version or 1):
            return {
                ...
                "errors": [
                    {
                        "code": "stale_version",
                        "message": "La location a changé. Recharge-la avant de demander le contrat.",
                    }
                ],
            }
        if doc.rental_state != "Reservation":
            frappe.throw(
                "Seule une réservation peut être soumise à l’approbation du contrat.",
                frappe.ValidationError,
            )
        ...
        return {
            ...
            "status": "approval_required",
            "approval_required": True,
            "approval_request_id": request.name,
            "mutation_performed": False,
        }
    ...
    def update_quote_draft():
        ...
        if doc.rental_state != "Quote":
            frappe.throw(
                "Seuls les brouillons de devis peuvent être modifiés.",
                frappe.ValidationError,
            )
        if int(payload.get("version") or 0) != int(doc.version or 1):
            return {
                ...
                "errors": [
                    {
                        "code": "stale_version",
                        "message": "Le devis a changé. Recharge-le avant de continuer.",
                    }
                ],
            }
        merged = {
        ...
        AuditService.record_mutation(
            company=company,
            action="cortex.rental_transaction.quote_updated",
            entity_type="Cortex Rental Transaction",
            entity_id=doc.name,
            before_state=before,
            after_state={
