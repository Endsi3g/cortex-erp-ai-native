...
describe('Navigation Store — Sidebar, Search & Breadcrumbs', () => {
  ...
  it('N-TEST-3: Records recent routes up to maximum 8 items without duplicates', () => {
    ...
    navStore.addRecentRoute({ name: 'operations-overview', path: '/operations', title: 'Cockpit' })
    navStore.addRecentRoute({ name: 'availability-matrix', path: '/availability', title: 'Matrice' })
    navStore.addRecentRoute({ name: 'operations-overview', path: '/operations', title: 'Cockpit' })
