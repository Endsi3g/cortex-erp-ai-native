...
describe('R10 Privacy & OwnerStatementSafe Isolation Tests (8 Mandatory Invariants)', () => {
  ...
  it('Assertion 8: Canonical route for Owner Statement uses /consignment/owners/:owner/statement/:period with no customer param', () => {
    const ownerStatementRoute = routes.find((r) => r.name === 'owner-statement' || r.meta?.screenId === 15)
    expect(ownerStatementRoute).toBeDefined()
    expect(ownerStatementRoute?.path).toBe('/consignment/owners/:owner/statement/:period')
