...
describe('Cortex OS — Router & Navigation Architecture (canonical routes)', () => {
  ...
  it('R-TEST-1: Defines every canonical screen with complete CortexRouteMeta', () => {
    // Screens 8, 16, 17 and 19 were merged into the AI Inbox / AI Audit /
    // Assistant by HANDOFF V2 and only survive as redirects.
    const merged = [8, 16, 17, 19]
    const canonicalScreenIds = Array.from({ length: 30 }, (_, i) => i + 1).filter(id => !merged.includes(id))
